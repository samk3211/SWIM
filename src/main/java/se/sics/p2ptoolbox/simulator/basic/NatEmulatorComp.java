...
public class NatEmulatorComp extends ComponentDefinition {
    ...
    private boolean allowMsg(Msg msg, NatedAddress src) {
        if(selfAddress.isOpen()) {
            return true;
        }
        NatedAddress selfAddressFromMessage = (NatedAddress) msg.getHeader().getDestination();
        LinkedList<Integer> parentsId = new LinkedList<Integer>();
        for(NatedAddress na : selfAddressFromMessage.getParents()){
            parentsId.add(na.getId());
        }
        return parentsId.contains(src.getId());
    }
